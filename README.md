...
**식약처 실데이터 모드 (선택)** — 사이트 우측 상단 **🔌 실데이터 연결**
정적 사이트는 CORS·키노출·http 때문에 data.go.kr을 직접 못 부릅니다. 무료 프록시(Cloudflare Worker)를
하나 띄우면 **식약처 기능성화장품 보고품목**을 실시간 조회해 리포트로 렌더링합니다. 미연결 시 데모(자동 생성) 모드.
→ 배포·연결법: [`proxy/README.md`](proxy/README.md)

> 재무·법인 등 식약처 범위 밖 항목은 `data_gap`(금융위/관세청 API 추가 연동 필요)으로 표기됩니다.
> 배치 수집·스냅샷 저장은 `collect.yml`(Actions + API 키)로 실행합니다.
